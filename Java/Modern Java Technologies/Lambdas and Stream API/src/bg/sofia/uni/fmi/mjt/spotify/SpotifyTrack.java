...
public record SpotifyTrack(String id, Set<String> artists, String name,
    ...
    public static SpotifyTrack of(String line) {
        ...
        String id = tokens[ID];
        Set<String> artists = Stream.of(tokens[ARTISTS]
                .split(";")).map(s -> s.strip().replace("[", "").replace("]", "").replaceAll("'", ""))
        ...
        return new SpotifyTrack(id, artists, name, year, popularity, duration, tempo,
